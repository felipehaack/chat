...
class jsonMacro(isStrict: Boolean) {
  def impl(c: whitebox.Context)(annottees: c.Expr[Any]*): c.Expr[Any] = {
    ...
    def modifiedEnumeration(compDecl: ModuleDef) = {
      try {
        val q"object $className extends Enumeration { ..$body }" = compDecl
        c.Expr( q"""
          object $className extends Enumeration {
            implicit val jsonAnnotationFormat = com.affin.chat.util.JsonUtil.enumValueFormat($className)
            ..$body
          }
          """)
      } catch {
        case _: MatchError => c.abort(c.enclosingPosition, "Annotation is only supported on enumeration")
      }
    }
    ...
    annottees.map(_.tree) match {
      ...
      case (compDecl: ModuleDef) :: Nil => modifiedEnumeration(compDecl)
